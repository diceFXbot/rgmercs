...
return {
    ...
    ['AbilitySets']   = {
        ...
        ['BigColdNuke'] = {                -- Level 60-70, Timed with great Ratio or High Cast Time/Damage
            ...
            "Ice Meteor",                  -- Level 64
            "Ancient: Destruction of Ice", -- Level 60, 13s recast
        },
        ['IceSpear'] = {                   -- 13s shared recast; one line, separate from ColdNuke
            "Ice Spear of Solist",         -- Level 60
    ...
    ['Helpers']       = {
        ...
        --- True while spell gems are locked by the global recast timer (between nukes).
        GemOnGlobalCooldown = function(_)
    ...
    ['RotationOrder'] = {
        ...
        },
        {
            ...
            state = 1,
            steps = 1,
            load_cond = function() return Core.GetResolvedActionMapItem('IceSpear') end,
            doFullRotation = true,
            targetId = function(self) return Targeting.CheckForAutoTargetID() end,
            cond = function(self, combat_state)
                return combat_state == "Combat" and not (Core.IsModeActive('PBAE') and Combat.AETargetCheck(true))
        ...
        },
        {
            ...
            steps = 1,
            load_cond = function()
                if Casting.CanUseAA("Force of Will") or Casting.CanUseAA("Lower Element") then return true end
                local weaveClickies = Modules:ExecModule("Clickies", "GetClickiesForRotation", "Weaves") or {}
                return #weaveClickies > 0
            end,
            targetId = function(self) return Targeting.CheckForAutoTargetID() end,
            cond = function(self, combat_state)
