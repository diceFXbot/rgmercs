...
local _ClassConfig = {
    ...
    ['Helpers']       = {
        ...
        DefensiveDiscCheck = function(printDebug)
            ...
            local discCount = Config:GetSetting('DiscCount')
            if xtCount < discCount then return false end
            local minConLevel = Config:GetSetting('DiscMinCon') or 1
            ...
            for i = 1, xtCount do
                ...
                if xtarg and xtarg.ID() > 0 and ((xtarg.Aggressive() or xtarg.TargetType():lower() == "auto hater")) and (xtarg.Distance() or 999) <= 30 and conLevel >= minConLevel then
                    if printDebug then
                        Logger.log_verbose("DefensiveDiscCheck(): XT(%d) Counting %s(%d) as a hater in range (ConColor=%s).", i, xtarg.CleanName() or "None", xtarg.ID(), xtarg.ConColor() or "None")
                    end
                    haters:add(xtarg.ID())
                end
                if #haters:toList() >= discCount then return true end -- no need to keep counting once this threshold has been reached
            end
            return false
        end,
    ...
    ['DefaultConfig'] = {
        ...
        ['DiscCount']       = {
            ...
            Category = "Defenses",
            Index = 101,
            Tooltip = "Number of auto haters in range (30) at or above Def. Disc. Min Con before preemptively using Defensive Discs.",
            Default = 4,
            Min = 1,
            Max = 10,
            ConfigType = "Advanced",
        ...
        ['DiscMinCon']      = {
            DisplayName = "Def. Disc. Min Con",
            Group = "Abilities",
            Header = "Tanking",
            Category = "Defenses",
            Index = 102,
            Tooltip = "Only count auto haters at or above this con color toward Def. Disc. Count.",
            Type = "Combo",
