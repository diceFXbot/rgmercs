...
local _ClassConfig = {
    _version              = "2.3 - EQ Might-kesh",
    _author               = "Algar, Derple, Robban",
    ['ModeChecks']        = {
        IsHealing = function() return true end,
        IsCuring = function()
            if Config:GetSetting('DoCureAA') or Config:GetSetting('DoCureSpells') then return true end
            if not Config:GetSetting('DoCureItems') then return false end
            for _, setting in ipairs({ 'CureItemPoison', 'CureItemDisease', 'CureItemCurse', 'CureItemCorruption', }) do
                if (Config:GetSetting(setting) or ""):len() > 0 then return true end
            end
            return false
        end,
    ...
    ['Helpers']           = {
        CureItemSettingForType = {
            poison = 'CureItemPoison',
            disease = 'CureItemDisease',
            curse = 'CureItemCurse',
            corruption = 'CureItemCorruption',
        },
        TryCureItem = function(self, type, targetId, targetSpawn)
            if not Config:GetSetting('DoCureItems') then return nil end

            local effectKey = (type or ""):lower()
            if effectKey == "mezzed" then return nil end

            local settingName = self.Helpers.CureItemSettingForType[effectKey]
            if not settingName then return nil end

            local itemName = Config:GetSetting(settingName) or ""
            if itemName:len() == 0 then return nil end

            if not mq.TLO.FindItem("=" .. itemName)() then
                Logger.log_debug("CureNow: Configured cure item '%s' for %s not found in inventory.", itemName, effectKey)
                return false, false
            end

            if not mq.TLO.Me.ItemReady(itemName)() then
                Logger.log_debug("CureNow: Cure item '%s' for %s is not ready.", itemName, effectKey)
                return false, false
            end

            Logger.log_debug("CureNow: Using item %s for %s on %s.", itemName, effectKey, targetSpawn.CleanName() or "Unknown")
            return Casting.UseItem(itemName, targetId), true
        end,
        ...
        ['GroupBuff'] = {
            ...
            },
            {
                ...
                cond = function(self, spell, target)
                    ...
                    if Targeting.TargetClassIs({ "WAR", "MNK", "ROG", "BER", }, target) then return false end
                    return Casting.GroupBuffCheck(spell, target)
                end,
            },
            {
    ...
    ['DefaultConfig']     = {
        ...
        ['DoCureItems']       = {
            DisplayName = "Do Cure Items",
            Group = "Abilities",
            Header = "Recovery",
            Category = "Curing",
            Index = 105,
            Tooltip = "After Cure AA, use configured cure items (Poison / Disease / Curse / Corruption), then cure spells if the item is unavailable.",
            Default = false,
            ConfigType = "Advanced",
        },
        ['CureItemPoison']    = {
            DisplayName = "Cure Item: Poison",
            Group = "Abilities",
            Header = "Recovery",
            Category = "Curing",
            Index = 106,
            Tooltip = "Item used to cure poison (cursor item onto button, same as Shrink Item).",
            Type = "ClickyItem",
            Default = "",
            ConfigType = "Advanced",
        },
        ['CureItemDisease']   = {
            DisplayName = "Cure Item: Disease",
            Group = "Abilities",
            Header = "Recovery",
            Category = "Curing",
            Index = 107,
            Tooltip = "Item used to cure disease.",
            Type = "ClickyItem",
            Default = "",
            ConfigType = "Advanced",
        },
        ['CureItemCurse']     = {
            DisplayName = "Cure Item: Curse",
            Group = "Abilities",
            Header = "Recovery",
            Category = "Curing",
            Index = 108,
            Tooltip = "Item used to cure curse.",
            Type = "ClickyItem",
            Default = "",
            ConfigType = "Advanced",
        },
        ['CureItemCorruption'] = {
            DisplayName = "Cure Item: Corruption",
            Group = "Abilities",
            Header = "Recovery",
            Category = "Curing",
            Index = 109,
            Tooltip = "Item used to cure corruption.",
            Type = "ClickyItem",
            Default = "",
            ConfigType = "Advanced",
        },
