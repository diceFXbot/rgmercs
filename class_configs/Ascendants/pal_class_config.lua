...
local _ClassConfig = {
    ...
    ['Cures']             = {
        GetCureSpells = function(self)
            ...
            -- Find the map for each cure spell we need, given availability of PurityCure. Fallback to dedicated lines.
            local neededCures = {
                ['Poison'] = Casting.GetFirstMapItem({ "PurityCure", "CurePoison", }),
                ['Disease'] = Casting.GetFirstMapItem({ "PurityCure", "CureDisease", }),
    ...
    ['AbilitySets']       = {
        ...
        ["YaulpSpell"] = {
            -- Yaulp self buff line
            "Yaulp XIX Rk. III",
            "Yaulp XIX Rk. II",
            "Yaulp XIX",   -- Level 125
            "Yaulp XVIII Rk. III",
            "Yaulp XVIII Rk. II",
            "Yaulp XVIII", -- Level 120
            "Yaulp XVII Rk. III",
            "Yaulp XVII Rk. II",
            "Yaulp XVII",  -- Level 115
            "Yaulp XVI Rk. III",
            "Yaulp XVI Rk. II",
            "Yaulp XVI",   -- Level 110
            "Yaulp XV Rk. III",
    ...
    ['HealRotations']     = {
        ["MainHealPoint"] = {
            ...
            },
            {
                ...
                type = "AA",
                cond = function(self, aaName, target)
                    return self.CombatState == "Combat" and Targeting.GetTargetPctHPs(target) < Config:GetSetting('LayHandsPct')
                end,
            },
            {
                name = "WaveHeal",
    ...
    ['Rotations']         = {
        ['Downtime'] = {
            ...
            },
            {
                ...
                cond = function(self, spell) --use this always until we have a Fury proc, and optionally after that, up until the point that Fury is rolled into DPU
                    if (mq.TLO.Me.AltAbility("Divine Protector's Unity").Rank() or 0) > 1 then return false end
        ...
        ['HateTools(AutoTarget)'] = {
            ...
            },
            {
                ...
                type = "Ability",
                cond = function(self, abilityName, target)
                    return (mq.TLO.Me.PctAggro() or 0) < 100 and target.ID() > 0 and Targeting.GetTargetDistance(target) < 30
                end,
            },
            {
                name = "ForHonor",
        ...
        ['Burn'] = {
            ...
            -- TODO: consider adding a tanking condition gate
            {
    ...
    ['SpellList']         = {
        {
            name = "Default",
            -- Priority-based loadout: fills available gems from top to bottom.
            spells = {
                { name = "ForHonor", },
                { name = "YaulpSpell", },
                { name = "StunTimer4", },
                { name = "TotLightHeal", },
                { name = "LightHeal", },
                { name = "PurityCure",       cond = function(self) return Config:GetSetting('MemCureSpell') end, },
                { name = "CurePoison",       cond = function(self) return not Core.GetResolvedActionMapItem('PurityCure') and Config:GetSetting('MemCureSpell') end, },
                { name = "CureDisease",      cond = function(self) return not Core.GetResolvedActionMapItem('PurityCure') and Config:GetSetting('MemCureSpell') end, },
                { name = "CureCurse",        cond = function(self) return not Core.GetResolvedActionMapItem('PurityCure') and Config:GetSetting('MemCureSpell') end, },
                { name = "CrushTimer6", },
                { name = "HealNuke", },
                { name = "SelfHeal", },
                { name = "BurstHeal", },
    ...
    ['DefaultConfig']     = {
        ...
        ['MemCureSpell']      = {
            ...
            Category = "Curing",
            Index = 101,
            Tooltip = "Mem your cure spells in the Spell Loadout.\n" ..
                "Prioritizes PurityCure; falls back to CureCurse if PurityCure is unavailable.",
            RequiresLoadoutChange = true,
            Default = true,
            ConfigType = "Advanced",
        },
