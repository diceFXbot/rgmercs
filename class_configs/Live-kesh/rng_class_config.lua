    ...
    ['Helpers']           = {
        rangedNav = function(reason)
            ...
            if Config:GetSetting('DoMelee') then
                if reason == "canthit" then
                    local target = mq.TLO.Target
                    local maxRange = (target.MaxRangeTo() or 999) < 30 and target.MaxRangeTo() or 10
                    local navDist = maxRange * 0.7
                    Logger.log_debug("rangedNav: canthit in melee mode, naving to %d away.", navDist)
                    Movement:NavInCombat(Globals.AutoTargetID, navDist, false, true, true)
                end
                return
            end
            ...
            if reason == "canthit" then
                Logger.log_debug("rangedNav: canthit, issuing simple nav to target.")
