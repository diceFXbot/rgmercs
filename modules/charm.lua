...
Module.DefaultConfig                      = {
    ...
    ['CharmSkipMATarget']                      = {
        ...
        Index       = 6,
        Default     = true,
        Tooltip     = "When enabled, the group Auto Target (MA target) will not be charmed.",
        ConfigType  = "Advanced",
    },
    -- Targets
    ['AutoLevelRangeCharm']                    = {
...
function Module:CastCharm(charmId)
    ...
    if Config:GetSetting('CharmSkipMATarget') and charmId == Globals.AutoTargetID then return end
...
function Module:IsValidCharmTarget(mobId)
    ...
    if Config:GetSetting('CharmSkipMATarget') and mobId == Globals.AutoTargetID then
        Logger.log_debug("\ayCharm: skipping MA auto target %d.", mobId)
        return false
    end
